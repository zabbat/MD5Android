...
public class Frame {
	...
	public Frame(float[] frameValues,int numJoints,Hierarchy[] hierarchy) {
		...
		for(int i=0;i<joints.length;i++){
			joints[i]=new Joint(hierarchy[i].name, hierarchy[i].parent, 
					...
					new Quaternion(frameValues[i*6+3],frameValues[i*6+4],frameValues[i*6+5]));
		}
	...
	private Pair<Quaternion, Vec3> mulByParent(Joint[] joints, Joint j){
		if(j.parent==-1){
			...
			return new Pair<Quaternion, Vec3>(j.q, j.position); 
		}else{
			...
			Quaternion newQuat=joints[j.parent].q.mul(j.q);
			newQuat.normalize();
